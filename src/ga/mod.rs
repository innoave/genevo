...
use statistic::{ProcessingTime, TimedResult, TrackProcessingTime, timed};
use chrono::Local;
use rayon;
...
impl<G, F, E, S, C, M, R> Algorithm for GeneticAlgorithm<G, F, E, S, C, M, R>
    where G: Genotype, F: Fitness + Send + Sync, E: FitnessFunction<G, F> + Sync,
          ...
          R: ReinsertionOp<G, F>
{
    ...
    fn next(&mut self, iteration: u64, rng: &mut Prng) -> Result<Self::Output, Self::Error>
    {
        ...
        if self.population.len() < self.min_population_size {
            return Err(GeneticAlgorithmError::PopulationTooSmall(
                format!("Population of generation {} has a size of {} which is smaller than the \
                        ...
                        iteration, self.population.len(), self.min_population_size)));
        }
        ...
        let selection = timed(||
            self.selector.select_from(&evaluation.result, rng)).run();
        let mut breeding = par_breed_offspring(selection.result, &self.breeder, &self.mutator, rng);
        let reinsertion = timed(||
            self.reinserter.combine(&mut breeding.result, &evaluation.result, rng)).run();

        // Stage 4: On to the next generation:
        let loop_time = evaluation.time + best_solution.time + selection.time + breeding.time
                      + reinsertion.time;
    ...
    where G: Genotype + Sync, F: Fitness + Send + Sync, E: FitnessFunction<G, F> + Sync
{
    ...
    let evaluated = EvaluatedPopulation::new(population, evaluation.result.0, evaluation.result.1,
                                             evaluation.result.2, average.result);
    TimedResult {
        result: evaluated,
...
fn determine_best_solution<G, F>(generation: u64, score_board: &EvaluatedPopulation<G, F>)
    -> TimedResult<BestSolution<G, F>>
    where G: Genotype, F: Fitness
{
    timed(|| {
        let evaluated = score_board.evaluated_individual_with_fitness(&score_board.highest_fitness())
...
fn par_breed_offspring<G, C, M>(parents: Vec<Parents<G>>, breeder: &C, mutator: &M, rng: &mut Prng)
    ...
    where G: Genotype + Send, C: CrossoverOp<G> + Sync, M: MutationOp<G> + Sync
{
    if parents.len() < 60 {
        ...
        let mut rng1 = rng.clone(); rng1.jump(1);
        let mut rng2 = rng.clone(); rng2.jump(2);
