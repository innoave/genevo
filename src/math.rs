...
use genetic::{ToScalar};


/// The `WeightedDistribution` is used to select values proportional to their
/// weighted values.
pub struct WeightedDistribution<'a, T> where T: 'a + ToScalar {
    values: &'a [T],
    sum: f64,
    weights: Vec<f64>,
}

impl<'a, T> WeightedDistribution<'a, T> where T: 'a + ToScalar {
...
fn calc_weights_and_sum<'a, T>(values: &'a [T]) -> (Vec<f64>, f64)
    ...
    for i in 0..values.len() {
        weights.push(values[i].to_scalar());
        weight_sum += values[i].to_scalar();
    }
    (weights, weight_sum)
}
